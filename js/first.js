    ...
    for (var i = 0; i < productsContainer.length; i++) {
        if (productsContainer[i].name.includes(term.trim()) == true) {
            cont += `<tr><td>` + productsContainer[i].name + `</td><td>` + productsContainer[i].price +
                `</td><td>` + productsContainer[i].category + `</td><td>` + productsContainer[i].description + `</td><td><button onclick="updateProduct(` + i + `)" class="btn btn-warning">Update</button></td>
                <td><button onclick="deleteProduct(` + i + `)" class="btn btn-danger">Delete</button></td>
                </tr>`;
            newTxt = productsContainer[i].name.replace(term, `<span class="font-weight-bold"  style="background-color: #eee;">` + term + `</span>`);
            cont2 += `<p class="m-0 border p-1">` + newTxt + `</p>`;
        }
    }
    if (term == "") {
        cont2 = `<p></p>`;
